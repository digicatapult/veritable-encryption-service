...
import { sha256HashFromBuffer } from '../../src/utils/hashing.js'
...
describe('Minio Encryption Service Integration', function () {
  ...
  describe('File Upload (Encryption Service)', function () {
    it('should upload a file successfully and return direct Minio URL', async () => {
      ...
      const fileHash = sha256HashFromBuffer(testFileContent)

      const response = await request(ENCRYPTION_SERVICE_URL)
        .post('/files/upload')
      ...
      expect(response.body.key).to.include(fileHash)
  ...
  describe('Direct Minio Access', function () {
    ...
    before(async () => {
      ...
      fileHash = sha256HashFromBuffer(testContent)
      const response = await request(ENCRYPTION_SERVICE_URL)
        .post('/files/upload')
    ...
    it('should allow direct download from Minio URL (anonymous access)', async () => {
      ...
      expect(urlParts.pathname).to.include(`test/${fileHash}`)
